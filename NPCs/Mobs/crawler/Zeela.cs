...
namespace MetroidMod.NPCs.Mobs.crawler
{
	public class Zeela : MNPC
	{
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			{
				Gore gore = Gore.NewGoreDirect(npc.Center, RandomVel, mod.GetGoreSlot("Gores/Mobs/ZeelaGore0"), npc.scale);
				gore.position -= new Vector2(Main.goreTexture[gore.type].Width,Main.goreTexture[gore.type].Height) / 2;
				gore.timeLeft = 60;
				gore = Gore.NewGoreDirect(npc.Center, RandomVel, mod.GetGoreSlot("Gores/Mobs/ZeelaGore1"), npc.scale);
				gore.position -= new Vector2(Main.goreTexture[gore.type].Width,Main.goreTexture[gore.type].Height) / 2;
				gore.timeLeft = 60;
				for(int i = 0; i < 4; i++)
				{
					gore = Gore.NewGoreDirect(npc.Center, RandomVel, mod.GetGoreSlot("Gores/Mobs/ZeelaGore2"), npc.scale);
					gore.position -= new Vector2(Main.goreTexture[gore.type].Width,Main.goreTexture[gore.type].Height) / 2;
					gore.timeLeft = 60;
					if(i < 2)
					{
						gore = Gore.NewGoreDirect(npc.Center, RandomVel, mod.GetGoreSlot("Gores/Mobs/ZeelaGore3"), npc.scale);
						gore.position -= new Vector2(Main.goreTexture[gore.type].Width,Main.goreTexture[gore.type].Height) / 2;
						gore.timeLeft = 60;
					}
				}
	...
	{
		public override string Texture => "MetroidMod/NPCs/Mobs/crawler/Zeela";
		public override void SetDefaults()
		{
			npc.scale = 0.85f;
			base.SetDefaults();
