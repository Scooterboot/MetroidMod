...
namespace MetroidMod.Common
{
	public class IdleTorizoMapLayer : ModMapLayer
	{
		public override void Draw(ref MapOverlayDrawContext context, ref string text)
		{
			...
			if (!MSystem.bossesDown.HasFlag(MetroidBossDown.downedTorizo) && !NPC.AnyNPCs(ModContent.NPCType<Torizo>()) && context.Draw(torizoHeadTexture.Value, new Vector2(room.Left + (room.Width / 2f), room.Top + (room.Height / 2f)), Color.White, new SpriteFrame(1, 1, 0, 0), scaleIfNotSelected, scaleIfSelected, Alignment.Center).IsMouseOver)
			{
