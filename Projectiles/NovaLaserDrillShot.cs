...
namespace MetroidMod.Projectiles
{
	public class NovaLaserDrillShot : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			Texture2D texture2D36 = Main.projectileTexture[P.type];
			...
			if (num274 > num277)
			{
				for (int num278 = 0; num278 < 4; num278++)
				{
					...
					if(num278 == 2)
					{
						spriteBatch.Draw(texture2D36, end - Main.screenPosition, new Rectangle(0,0,texture2D36.Width,(int)(num274-num277)/2), alpha7, rot, new Vector2(texture2D36.Width, texture2D36.Height) / 2f, new Vector2(0.75f,2f), spriteEffects, 0f);
					}
					...
					for(int i = 0; i < dist-1; i++)
					{
						...
						spriteBatch.Draw(Main.magicPixel, pos[i] - Main.screenPosition, new Rectangle(0,0,1,1), white3, rot2, Vector2.One / 2f, new Vector2(2f,2f), spriteEffects, 0f);
					}
					
					spriteBatch.Draw(Main.magicPixel, midpos - Main.screenPosition, new Rectangle(0,0,1,(int)Vector2.Distance(midpos,end)), white3, rot3, Vector2.One / 2f, new Vector2(2f,1f), spriteEffects, 0f);
				}
			}
