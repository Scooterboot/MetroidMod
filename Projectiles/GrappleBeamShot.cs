	...
	public class GrappleBeamShot : ModProjectile
	{
		...
		public override bool PreDraw(SpriteBatch s, Color lightColor)
		{
			...
			for(int j = 0; j < 3; j++)
			{
				...
				{
					this.DrawChain(owner.Center, projectile.Center, mod.GetTexture("Projectiles/GrappleBeamChain"), s, chainFrame, 4);
				}
			}
			tex = Main.projectileTexture[P.type];
			int num = tex.Height / Main.projFrames[projectile.type];
		...
		public void DrawChain(Vector2 start, Vector2 end, Texture2D name, SpriteBatch spriteBatch, int frame = 0, int frameCount = 0)
		{
