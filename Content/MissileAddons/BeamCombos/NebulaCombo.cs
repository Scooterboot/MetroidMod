	...
	public class NebulaCombo : ModMissileAddon
	{
		...
		public override void AI(MProjectile mProjectile)
		{
			...
			if (!Initialized && P.owner == Main.myPlayer)
			{
				...
				{
					int b = Projectile.NewProjectile(entitySource, P.Center.X, P.Center.Y, 0f, 0f, ModContent.ProjectileType<NebulaBusterShot>(), (int)(P.damage * 0.25f), P.knockBack, P.owner);
		...
		public override bool PreDrawProjectile(MProjectile mProjectile, ref Color lightColor)
		{
			...
			tex2 = ModContent.Request<Texture2D>($"{Mod.Name}/Content/MissileAddons/BeamCombos/NebulaComboShot2").Value;
			...
			if (Lead != null && Lead.active)
			{
				...
				Texture2D tex3 = Terraria.GameContent.TextureAssets.Projectile[P.type].Value;
				int num108 = tex3.Height / 2;
				int y4 = num108 * P.frame;

				Texture2D tex4 = ModContent.Request<Texture2D>($"{Mod.Name}/Content/MissileAddons/BeamCombos/NebulaComboShot2").Value;
