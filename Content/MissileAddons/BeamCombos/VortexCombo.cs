...
namespace MetroidMod.Content.MissileAddons.BeamCombos
{
	public class VortexCombo : ModMissileAddon
	{
		...
		public override void AI(MProjectile mProjectile)
		{
			...
			mProjectile.Explode((int)(8f * P.scale));
		}
		public override void OnKill(MProjectile mProjectile, int timeLeft)
		{
			...
			{
				int v = Projectile.NewProjectile(P.GetSource_Death(), P.Center.X, P.Center.Y, velocity.X, velocity.Y, ModContent.ProjectileType<VortexComboShot2>(), damage, P.knockBack, P.owner);
		...
		{
			if (mProjectile.Projectile.velocity.X != oldVelocity.X)
			{
				mProjectile.Projectile.velocity.X = -oldVelocity.X;
			}
			if (mProjectile.Projectile.velocity.Y != oldVelocity.Y)
			{
				mProjectile.Projectile.velocity.Y = -oldVelocity.Y;
			}
			return false;
		}
		...
		public override bool PreDrawProjectile(MProjectile mProjectile, ref Color lightColor)
		{
			...
			Texture2D tex2 = Terraria.GameContent.TextureAssets.Extra[ExtrasID.VortexBlack].Value;
