...
{
	public class ShockCoil : ModBeamAddon
	{
		public override bool AddOnlyAddonItem => false;
		public override int ShotDust => 110;
		public override Color PrimaryColor => MetroidMod.lumColor;
		public override void SetStaticDefaults()
		{
			AddonSlot = BeamAddonSlotID.Primary;
			VIB = true;
			vibOverride = ModContent.ProjectileType<ShockCoilShot>();
			ArrayPassive = false;
			Item.ResearchUnlockCount = 1;
			ItemID.Sets.ShimmerTransformToItem[Type] = BeamAddonLoader.GetAddon<BattleHammer>().ItemType;
		}
		...
		public override void VIBShoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, string bonusFileMod = "", float multiplier = 1)
		{
			...
			ShockCoilShot tasteTheRainbow = Projectile.NewProjectileDirect(source, position, velocity, ModContent.ProjectileType<ShockCoilShot>(), damage, knockback).ModProjectile as ShockCoilShot;
	...
	public class ShockCoilShot : MProjectile
	{
		...
		public override string Texture => $"{Mod.Name}/Assets/Textures/BeamAddons/ShockCoil/Shot";
		...
		public override void AI()
		{
			...
			if (P.owner == Main.myPlayer && !O.dead)
			{
				...
				if (P.numUpdates == 0)
				{
					if (soundDelay <= 0)
					{
						if (!soundPlayed)
						{
							SoundEngine.TryGetActiveSound(SoundEngine.PlaySound(new($"{Mod.Name}/Assets/Sounds/BeamAddons/ShockCoil/Loop"), O.position), out ActiveSound result);
						...
						{
							SoundEngine.TryGetActiveSound(SoundEngine.PlaySound(new($"{Mod.Name}/Assets/Sounds/BeamAddons/ShockCoil/ShotCharged"), O.position), out ActiveSound result);
							soundInstance = result.Sound;
							soundDelay = 40;
						}
			...
			if (mp.statOverheat >= mp.maxOverheat || O.HeldItem.GetGlobalItem<MGlobalItem>().statUA <= 0)//O.HeldItem.GetGlobalItem<MGlobalItem>().addonUACost)
			{
				P.Kill();
				mp.statCharge = 0;
				SoundEngine.PlaySound(new($"{Mod.Name}/Assets/Sounds/BeamAddons/ShockCoil/Reload"), O.position);
			}
		}
		...
		public override void OnHitNPC(NPC target2, NPC.HitInfo hit, int damageDone)
		{
			...
			float minDamage = .5f;// MConfigItems.Instance.minSpeedShockCoil;// + (Luminite? 1.0f : DiffBeam? 0.5f : 0f);
			float maxDamage = 1.5f;// MConfigItems.Instance.maxSpeedShockCoil + (Luminite ? 1.0f : DiffBeam ? 0.5f : 0f);
			...
			//	}
			//}
			SoundEngine.PlaySound(new($"{Mod.Name}/Assets/Sounds/BeamAddons/ShockCoil/Impact"), Projectile.position);
			if (damageDone > 0)
			{
