...
namespace MetroidMod.Content.BeamAddons
{
	public class VoltDriver : ModBeamAddon
	{
		...
		public override void VIBShoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, string bonusFileMod = "", float multiplier = 1)
		{
			...
			VoltDriverShot tasteTheRainbow = Projectile.NewProjectileDirect(source, position, velocity, ModContent.ProjectileType<VoltDriverShot>(), damage, knockback).ModProjectile as VoltDriverShot;
	...
	public class VoltDriverShot : MProjectile
	{
		...
		public override string Texture => $"{Mod.Name}/Assets/Textures/BeamAddons/VoltDriver/Shot";
	...
	{
		public string fileMod = "";
		public override string Texture => $"{Mod.Name}/Assets/Textures/BeamAddons/VoltDriver/ShotCharged";
		public ModBeamAddon[] beamAddons = new ModBeamAddon[BeamAddonSlotID.Count - 1];
		public override void SetStaticDefaults()
		{
			Main.projFrames[Projectile.type] = 4;
