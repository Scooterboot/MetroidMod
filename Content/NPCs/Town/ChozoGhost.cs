...
namespace MetroidMod.Content.NPCs.Town
{
	...
	public class ChozoGhost : ModNPC
	{
		...
		private string QuestChat(NPC npc)
		{
			...
			if (player.armor[1].type == ModContent.ItemType<PowerSuitBreastplate>() && (player.armor[1].ModItem as PowerSuitBreastplate).SuitAddons[(int)BreastplateAddonSlot.Barrier] == SuitAddonLoader.GetAddon<VariaSuitAddon>().GeneratedModItem.Item)
			{
