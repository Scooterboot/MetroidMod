...
namespace MetroidModPorted.Content.NPCs.Town
{
	...
	public class ChozoGhost : ModNPC
	{
		...
		string QuestChat(NPC npc)
		{
			...
			if(player.armor[1].type == ModContent.ItemType<PowerSuitBreastplate>() && (player.armor[1].ModItem as PowerSuitBreastplate).SuitAddons[SuitAddonSlotID.Suit_Varia] == SuitAddonLoader.GetAddon<VariaSuitAddon>().Item.Item)
			{
