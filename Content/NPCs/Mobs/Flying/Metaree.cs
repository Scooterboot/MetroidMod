			...
			if (NPC.ai[0] == 2) // NPC id burrowing.
			{
				...
				if (NPC.ai[1]++ <= 60) // 'Spew' projectiles.
				{
					if (Main.netMode != 1 && NPC.ai[1] % 15 == 0)
						Projectile.NewProjectile(NPC.GetSource_FromAI(), NPC.Center, new Vector2(Main.rand.Next(-40, 41) * .1F, -5), ModContent.ProjectileType<Projectiles.Mobs.MetareeRock>(), NPC.damage, 1.2F);
				}
				else
					NPC.active = false;
			}
